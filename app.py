...
<style>
    ...
    * { color: var(--text-white); }
    ...
    .stButton > button:hover, .btn-oauth:hover {
...
def extrair_dados_nf(pdf_bytes, nome_arquivo):
    try:
        with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
            ...
            dados = {
                "NF": extrair_nf(texto_completo, whites:=linhas),
                "Data de emissão": "N/A",
...
if uploaded_files:
    ...
    with tab3:
        ...
        if "code" in parametros_url and "token_notion_usuario" not in st.session_state:
            ...
            with st.spinner("Validando credenciais corporativas no Notion..."):
                ...
                if resposta_oauth:
                    ...
                    # CAPTURA AUTOMÁTICA DO ID ESPECÍFICO DO USUÁRIO CORRENTE
                    if "duplicated_template_id" in resposta_oauth and resposta_oauth["duplicated_template_id"]:
                        st.session_state["id_notion_automatico"] = resposta_oauth["duplicated_template_id"]
        ...
        if "token_notion_usuario" not in st.session_state:
            ...
            # Busca automática complementar caso o template_id não venha na resposta original
            ...
            if id_final_tabela:
                if st.button("🚀 TRANSMITIR REGISTROS PARA O MEU NOTION"):
                    ...
                    if falhas == 0:
                        ...
                        st.error(f"🔴 Erro no envio: {falhas} falhas detectadas. Verifique se o seu Notion possui acesso à tabela.")
            else:
